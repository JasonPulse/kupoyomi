 ...
 */
const scratch = (): string => mkdtempSync(join(tmpdir(), "cbz-"));
...
test("an archive we write reads back byte for byte", async () => {
  ...
  const cbz = buildCbz([
    { name: "ComicInfo.xml", data: comicInfo({ series: "Test & Co", number: "12.5", pageCount: 2,
        uploaded: new Date("2026-05-10T00:00:00Z") }) },
    ...pages,
...
test("deflated entries are inflated, since adopted files are not ours", async () => {
  ...
  local.writeUInt32LE(0, 14); local.writeUInt32LE(deflated.length, 18);
  local.writeUInt32LE(payload.length, 22); local.writeUInt16LE(name.length, 26);
  name.copy(local, 30);

  const central = Buffer.alloc(46 + name.length);
  central.writeUInt32LE(0x02014b50, 0); central.writeUInt16LE(20, 4); central.writeUInt16LE(20, 6);
  central.writeUInt16LE(8, 10);
  central.writeUInt32LE(0, 16); central.writeUInt32LE(deflated.length, 20);
  central.writeUInt32LE(payload.length, 24); central.writeUInt16LE(name.length, 28);
  central.writeUInt32LE(0, 42); name.copy(central, 46);

  const cdOffset = local.length + deflated.length;
  const end = Buffer.alloc(22);
  end.writeUInt32LE(0x06054b50, 0); end.writeUInt16LE(1, 8); end.writeUInt16LE(1, 10);
  end.writeUInt32LE(central.length, 12); end.writeUInt32LE(cdOffset, 16);
